	...
	public function insert_answer_comment($answer_id, $uid, $message)
	{
		...
		if ($answer_info['uid'] != $uid)
		{
			...
			if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($answer_info['uid']))
			{
				...
				{
					$this->model('weixin')->send_text_message($weixin_user['openid'], "您在 [" . $question_info['question_content'] . "] 中的回答收到了新评论:\n\n" . strip_tags($message), $this->model('openid_weixin_weixin')->redirect_url('/question/' . $question_info['question_id'] . '?answer_id=' . $answer_info['answer_id'] . '&single=TRUE'));
				}
			}
		}
			...
			foreach ($at_users as $user_id)
			{
				if ($user_id != $uid)
				{
					...
					if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($user_id))
					{
						...
						$this->model('weixin')->send_text_message($weixin_user['openid'], $answer_user['user_name'] . " 在问题 [" . $question_info['question_content'] . "] 的答案评论中提到了您", $this->model('openid_weixin_weixin')->redirect_url('/question/' . $question_info['question_id'] . '?answer_id=' . $answer_info['answer_id'] . '&single=TRUE'));
					}
				}
			}
		}
