...
class question_class extends AWS_MODEL
{
	...
	public function parse_at_user($content, $popup = false, $with_user = false, $to_uid = false)
	{
		...
		if (is_array($matchs[1]))
		{
			...
			foreach ($match_name as $key => $user_name)
			{
				...
				if ($user_info)
				{
					$content = str_replace('@' . $user_name, '<a href="people/' . $user_info['url_token'] . '"' . (($popup) ? ' target="_blank"' : '') . ' class="user-name" data-id="' . $user_info['uid'] . '">@' . $user_info['user_name'] . '</a>', $content);
	...
	public function get_question_filter_query_info($sort = null, $category_id = null, $difficulty = null, $quiz_type = null, $countdown = null, $urecord = null, $date = null, $uid = 0)
	{
	...
	public function get_homepage_recommend_question_list_total()
	{
